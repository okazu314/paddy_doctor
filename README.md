...
# 概要
## 問題意識
米（Oryza sativa）は世界的な主食の1つです。籾は、籾殻を除去する前の生の穀物であり、主にアジア諸国の熱帯気候で栽培されています。水稲栽培は、病気や害虫の影響を受けやすく、最大で70％の収量減につながるため、一貫した管理が必要です。これらの病害を軽減し、作物の損失を防ぐには、通常、専門家の監督が必要です。作物保護の専門家が限られているため、手作業による病害の診断は面倒で費用もかかります。そこで、様々な分野で有望な成果を上げているコンピュータビジョンに基づく技術を活用し、病害虫の特定プロセスを自動化することがますます重要になってきています。

## 目的
本コンペティションの主な目的は、与えられた水稲の葉の画像を正確に分類するための機械学習または深層学習に基づいたモデルを開発することです。データは、10クラス（9つの病気カテゴリと正常葉）にまたがる10,407枚（75％）のラベル付き画像のトレーニングデータセットを使用します。さらに、各画像に対して、籾の品種や年齢などの付加的なメタデータも提供されています。最終的なタスクは、与えられた3,469枚（25%）のテストデータセット中の各水田画像を、9つの病気カテゴリと正常な葉のいずれかに分類することです。

## 正常な稲の画像
![image](https://user-images.githubusercontent.com/74422064/177743064-bcbc80b0-143c-452f-bcac-6ec5fd20468b.png)
## 病気にかかった稲の画像
![image](https://user-images.githubusercontent.com/74422064/177743375-941cfc8a-57d6-4096-8aa1-8c173b5e5d51.png)
...
- 5/27 mainプログラムでのエラーがゼロ。改善のアイデアだし。
    - エポック数を20から30に変更
    - モデルDeiT（timmで使えてそう）
    - 最適化関数AdamからAdamw(安定，早め)

- 6/3 いろいろなモデル試す．パラメータの変更による結果の影響見る
    - モデルCaiTを試してみる
    - 最適化関数AdamWの学習率をデフォルト値とそこから0.1下げて実験

- 6/10 いろいろなモデルを試す．
    - モデルCaiTでRAdam試す．学習率を少し小さくする．バッチサイズ32
    - モデルSwin, CoaTも試してみる（バッチサイズ32無理なら16)
    -　バッチサイズ疑似的に10倍に上げる機能を実装する

- 6/17 今までに結果出てるモデルのパラメータ変更
    - モデルSwinのバッチサイズあげる
    - efficientnet-b4でバッチサイズ上げる

- 6/24(最終日)
    - Swinで学習率などのパラメータ変更した実験．全ての通りためす   
    - efficientnet-b5の実装
    - resnet50の実装
...
- 報告が早かった
    - リーダーのレスポンスと決断が早かった
    - 似た問題の抑制(KaggleNotebookの制限時間の問題)
- 自主的にモデルを探して実験を行うことができた
    - 約50のパターンで機械学習モデルを作成できた
- 役割を上手く分担できた
- 後半の議論の時間を長引かせないようにした
    - 時間を決めて話し合った

悪かった点
- 目の前のことにしか意識が行かなかった
    - ゴールを見据えて開発を進める
- 忙しいときと忙しくないときでタスクの振り分けを行う
    - 忙しくないときに重めのタスクに取り組む
